...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=32),
            nullable=False,
            server_default="simulated_meter",
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_energy_metrics_voltage_non_negative",
        ),
        sa.CheckConstraint(
            "current_a IS NULL OR current_a >= 0",
            name="ck_energy_metrics_current_non_negative",
        ),
        sa.CheckConstraint(
            "power_factor IS NULL OR power_factor BETWEEN -1 AND 1",
            name="ck_energy_metrics_power_factor_range",
        ),
        sa.ForeignKeyConstraint(
            ["device_id"],
            ["devices.id"],
            ondelete="CASCADE",
            name="fk_energy_metrics_device_id_devices",
        ),
        sa.ForeignKeyConstraint(
            ["home_id"],
            ["homes.id"],
            ondelete="CASCADE",
            name="fk_energy_metrics_home_id_homes",
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="fk_energy_metrics_user_id_users",
        ),
        sa.PrimaryKeyConstraint("device_id", "ts", name="pk_energy_metrics"),
    )

    op.execute("SELECT create_hypertable('energy_metrics', 'ts', if_not_exists => TRUE)")
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["homes.id"],
            ondelete="CASCADE",
            name="fk_anomalies_home_id_homes",
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="fk_anomalies_user_id_users",
        ),
